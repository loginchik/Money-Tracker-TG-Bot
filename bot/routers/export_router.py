...
@export_router.message(Command('export_my_data'), UserExists(), StateFilter(None))
async def export_users_data(message: Message, user_lang: str, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        expenses_data = await db.expense_operations.get_user_expenses(user_id, user_lang)
    ...
    try:
        incomes_data = await db.income_operations.get_user_income(user_id)
