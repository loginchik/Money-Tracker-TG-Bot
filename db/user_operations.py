...
async def user_exists(user_id: int, db_connection: asyncpg.Connection) -> bool:
    """
    ...
    exists = await db_connection.fetchval(users_table_query, user_id)
    # User expenses table exists
    table_query = '''SELECT EXISTS (SELECT FROM information_schema.tables 
    WHERE table_schema = $1 AND table_name = $2);'''
    expense_table_exists = await db_connection.fetchval(table_query, 'user_based', f'expense_{user_id}')
    # User incomes table exists
    income_table_exists = await db_connection.fetchval(table_query, 'user_based', f'income_{user_id}')
    # User expense limits table exists
    limits_table_exists = await db_connection.fetchval(table_query, 'user_based', f'expense_limit_{user_id}')

    return all([exists, expense_table_exists, income_table_exists, limits_table_exists])


async def create_user(db_connection: asyncpg.Connection, **user_data: dict) -> bool:
    """
